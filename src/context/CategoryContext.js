...
export class CategoryProvider extends React.Component {
  ...
  handleChange = (event) => {
    ...
    const categoryIndex = modifiedCategories.findIndex(category => category.id === parseInt(categoryId))

    modifiedCategories[categoryIndex].checked = !modifiedCategories[categoryIndex].checked
  ...
  toggleAll = () => {
    ...
    const isAllChecked = categories.every(category => category.checked === true)
